"""Tutorial 30: Qwen2.5-1.5B-Instruct inference on GSM8K — short generation.

Calibration target bucket (llm_inference, medium msc, small-to-medium dps,
small cps, one epoch, tiny samples). Model fits on small GPUs; total
generated tokens kept under 1000 to stay in `samples_bucket=tiny`.

Expected runtime: ~30-60 s on RTX 4090, 80 samples × 100 tokens.
"""
...
HF_MODEL = "hf://models/Qwen/Qwen2.5-1.5B-Instruct"
...
async def main():
    ...
    print("Submitting Qwen2.5-1.5B inference (short Q/A)...")
